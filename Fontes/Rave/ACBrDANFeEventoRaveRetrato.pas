...
begin
  ...
  begin
    if ( InfEvento.tpAmb = taHomologacao ) then
     ...
     Box([],PosX,PosY,109.8,aHeigthPadrao,'CHAVE DE ACESSO',FormatarChaveAcesso(InfEvento.chNFe),taRightJustify);
     NewLine;
     NewLine;
     NewLine;
     PosX := 10;
      ...
      begin
        if EventoRave.ACBrNFe.NotasFiscais.Items[0].NFe.Emit.CNPJCPF = InfEvento.CNPJ then
         begin
           ...
           Box([],XPos,YPos, 22,aHeigthPadrao,'CEP', FormatarCEP(Poem_Zeros(EventoRave.ACBrNFe.NotasFiscais.Items[0].NFe.Emit.EnderEmit.CEP,8)),taRightJustify);
           PosY := YPos + aHeigthPadrao;
        ...
        if (EventoRave.ACBrNFe.NotasFiscais.Items[0].NFe.infNFe.ID = InfEvento.chNFe) or
         ...
         begin
           ...
           Box([],XPos,YPos, 22,aHeigthPadrao,'CEP', FormatarCEP(Poem_Zeros(EventoRave.ACBrNFe.NotasFiscais.Items[0].NFe.Dest.EnderDest.CEP,8)),taRightJustify);
           PosY := YPos + aHeigthPadrao;
     ...
     SetFontTitle;
     PrintXY(PosX,FLastY,'DATA E HORA DA IMPRESSÃO: '+FormatDateTime('dd/mm/yyyy hh:mm:ss',Now) + ' ' + NomeDoUsuario);
